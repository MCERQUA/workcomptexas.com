...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${loc.blurb} Workers' comp for Texas contractors in ${loc.name} — GC, roofing, HVAC, construction, and specialty trades. 15-minute quotes.`,
  ...
  return (
    <>
      ...
      <main>
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-center">
              ...
              <FadeIn direction="left" className="lg:col-span-5">
                <div className="rounded-t-[10rem] rounded-b-3xl overflow-hidden border-4 border-white shadow-warm-lg">
                  <img src="/images/coverage.jpg" alt={`Texas contractors working in ${loc.name}`} className="w-full h-[300px] md:h-[360px] object-cover" loading="lazy" />
                </div>
              </FadeIn>
            </div>
          </div>
        ...
        <section className="bg-cream py-16 md:py-20">
          <div className="container-tight">
            <FadeIn className="max-w-2xl mb-10">
              <span className="eyebrow"><span className="h-px w-8 bg-clay" />Texas workers&apos; comp for {loc.name} contractors</span>
              <h2 className="mt-3 h-section">WC programs built for {loc.name} contractor trades.</h2>
              <p className="mt-4 lead">From a sole sub with 2 employees to a GC managing multiple crews, we write the right WC program for your {loc.name} operation.</p>
            </FadeIn>
        ...
        <FAQ items={locFaqs} eyebrow={`${loc.name} — FAQ`} title={<>Texas workers&apos; comp questions for <span className="text-clay">{loc.name}</span> contractors</>} background="sand" />

        <CTABand title={`Writing Texas workers' comp in ${loc.name} since 2005`} description={`Correct class codes, A-rated carriers, and 15-minute quotes. Call ${SITE.phone} or request a quote online.`} />
      </main>
      <Footer />
    </>
  );
